    ...
    if (parallel) {
        #pragma omp parallel shared(ptr, buckets), private(seed, timerStart)
        {
            ...
            if (omp_get_thread_num() == 0) {
                printf("0_random_generating:%lf", (timerEnd - timerStart));
            }
            ...
            timerEnd = omp_get_wtime();
            if (omp_get_thread_num() == 0) {
                printf(",1_buckets_ins_filling_(squash):%lf", (timerEnd - timerStart));
            }
            ...
            timerEnd = omp_get_wtime();
            if (omp_get_thread_num() == 0) {
                printf(",2_buckets_sorting:%lf", (timerEnd - timerStart));
            }
            ...
            timerEnd = omp_get_wtime();
            if (omp_get_thread_num() == 0) {
                printf(",3_buckets_to_main_array:%lf", (timerEnd - timerStart));
            }
        }
